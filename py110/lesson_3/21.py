...
def deal(cards):
    ...
    cards[DEALER_HIDDEN] = [0, cards[DEALER][FIRST_CARD], HIDDEN_CARD]
...
def display_hands(cards, dealer):
    system("clear")
    print("Dealer:")
    print(f"[{']['.join(cards[dealer][FIRST_CARD:])}] Total: {cards[dealer][HAND_SUM]}")
    print("\nPlayer:")
    print(f"[{']['.join(cards[PLAYER][FIRST_CARD:])}] Total: {cards[PLAYER][HAND_SUM]}")
